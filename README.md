...
4. `php artisan zenphone:preflight` - must print "All critical checks passed".
...
9. Register on the site, then `php artisan zenphone:make-admin you@example.com` to reach `/admin`.
...
3. SSH (or hPanel's terminal), from `public_html`: `php artisan key:generate --force && php artisan migrate --force && php artisan storage:link && php artisan zenphone:preflight`.
